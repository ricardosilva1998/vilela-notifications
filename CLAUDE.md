...
- Bridge always-on-top re-assert (v3.27.0+): a single shared `_globalTopInterval` in `bridge/main.js` iterates `overlayWindows` every 2s and calls `setAlwaysOnTop(true, 'screen-saver')` for each. Replaces the old per-overlay setInterval-per-window pattern. `ensureGlobalTopInterval()` starts it on overlay creation; `maybeStopGlobalTopInterval()` clears it when the last overlay closes. Keep it as one timer — N overlays × N timers was wasted work for identical cadence.
